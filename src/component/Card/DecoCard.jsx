...
const DecoCard = (props) => {
  ...
  // Construye la ruta en función del tipo
  const linkType = props.type === "navidad" ? "navidad" : "deco"; // Cambia "deco" por defecto si no es navidad

  return (
    <div className="project-card" style={{ position: 'relative', borderRadius: '10px', overflow: 'hidden' }}>
      <Link to={`/${linkType}/${props.id}`} style={{ textDecoration: "none" }}>
        {primeraImagen ? (
          <img
            src={primeraImagen}
            alt="imagen"
